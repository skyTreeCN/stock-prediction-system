...
export default function DemoPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // 下载PDF
      const stock = MOCK_PREDICTIONS[currentStockIndex]
      const fileName = `股票推荐_${stock.name}_${stock.code}_${new Date().toISOString().split('T')[0]}.pdf`
      pdf.save(fileName)
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-t-2xl shadow-lg">
          ...
          <div className="p-8">
            ...
            {activeTab === 'data' && (
              <div>
                <h2 className="text-2xl font-bold mb-4 text-blue-600">数据获取</h2>
                <p className="text-gray-600 mb-4">保持现有功能不变...</p>
                <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
                  <p className="text-sm text-gray-700">这里是原来的数据获取界面，功能不变</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'patterns' && (
              <div>
                <div className="mb-6">
                  <h2 className="text-2xl font-bold text-purple-600 mb-2">上涨模式分析</h2>
                  <p className="text-gray-600">点击模式展开查看30天完整数据和量化指标</p>
                </div>
                ...
                <div className="mb-6 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600">
                      共 {MOCK_PREDICTIONS.length} 只 | 当前第 {currentStockIndex + 1} 只
                    </p>
                  </div>
...
function KLineChart({
  ...
  // 对于750天数据，使用更合理的宽度计算
  const candleWidth = Math.max(2, 5) // 每根K线固定5像素宽
  const totalWidth = Math.max(900, data.length * (candleWidth + 2)) // 至少900px
  const leftMargin = 50
    ...
    <div className="relative">
      <svg width={totalWidth} height="280" viewBox={`0 0 ${totalWidth} 280`} className="overflow-visible">
        ...
        {data.map((candle, index) => {
          const x = leftMargin + index * (candleWidth + 2)
...
function VolumeChart({
  ...
  const candleWidth = 5
  const totalWidth = Math.max(900, data.length * (candleWidth + 2))
  const leftMargin = 50
  const rightMargin = 50
